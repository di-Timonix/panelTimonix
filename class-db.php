...
/* 
	...
	klasa db zarządza połączeniami z bazą danych
	...
	class db {
		...
		function __construct($host = null, $db_user = null, $db_password = null, $db_name = null, $db_prefix = null){
			...
			// otwieranie nowego połączenia z bazą blackmin
				$this->db_mysql = new mysqli($this->host, $this->db_user, $this->db_password, $this->db_name);
			}
		}
		...
		// funkcjia zamieniająca prefix dev na prefix db
		private function prefix_db($t){
			...
			// zastępienie prefixów dev na prefix db i zwracanie wyniku
		...
		public function array_flatten($array) {
		   ...
		   foreach ($array as $key => $value) {
			   if (is_array($value)){
				   $return = array_merge($return, db::array_flatten($value));
			   }else{
				   $return[$key] = $value;
				}
		   }
		...
		public function query($query, $isjson = false, $data_output = null){
			...
			try 
			{
				if ($this->db_mysql->connect_errno!=0)
				{
					throw new Exception(mysqli_connect_errno());
				}
				else
				{		
					mysqli_query($this->db_mysql, "SET CHARSET utf8");
					mysqli_query($this->db_mysql, "SET NAMES 'utf8' COLLATE 'utf8_unicode_ci'");		

					// sprawdzanie czy dane trzeba sparsować 
					if($this->set_valid == true){
						// parsowanie danych
						$t = htmlentities(db::prefix_db($query), ENT_QUOTES, "UTF-8");
						$t = htmlspecialchars($t, ENT_QUOTES, "UTF-8");
					}else{
						$t = db::prefix_db($query);
					}				
					
					if ($rezultat = $this->db_mysql->query(sprintf($t))) {
		...
		public function query2($query, $isjson = false, $data_output = null){
			...
			try 
			{
				...
				{		
					...
					if ($rezultat = $this->db_mysql->query($t)) {
		...
		public function insert($query, $isjson = false, $message = null){
			...
			try 
			{
				...
				{		
					...
					if ($this->db_mysql->query(sprintf($t))) {
						...
						throw new Exception($this->db_mysql->error);
					}
				}
